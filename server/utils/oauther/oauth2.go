...
func (o *oAuther) GetToken(userID string) (*oauth2.Token, error) {
	...
	if !token.Valid() {
		// TODO: is there a context.Context that can be re-used?
		ts := o.config.TokenSource(context.Background(), token)
		token, err = ts.Token()
		if err != nil {
			return nil, err
		}

		_, err := o.store.Set(o.getTokenKey(userID), &token)
		if err != nil {
			return nil, err
		}
	}
...
func (o *oAuther) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	switch r.URL.Path {
	case o.oAuthURL + ConnectPath:
		o.oauth2Connect(w, r)
	case o.oAuthURL + CompletePath:
