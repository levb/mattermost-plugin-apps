...
import (
	...
	"github.com/mattermost/mattermost-plugin-apps/server/mock/mock_appservices"
)
...
func TestKV(t *testing.T) {
	...
	r := mux.NewRouter()
	Init(r, nil, conf, nil, appservices)
	...
	require.NoError(t, err)
	appservices.EXPECT().KVSet(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
	...
	req.Header.Set("Mattermost-User-Id", "01234567890123456789012345")
	require.NoError(t, err)
	appservices.EXPECT().KVGet(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
		func(botUserID, prefix, id string, ref interface{}) (bool, error) {
			require.Equal(t, "01234567890123456789012345", botUserID)
			require.Equal(t, "", prefix)
			require.Equal(t, "test-id", id)
